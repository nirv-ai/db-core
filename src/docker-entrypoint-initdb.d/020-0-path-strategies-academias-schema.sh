...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$DEFAULT_DB" <<-EOSQL
  ...
  CREATE INDEX paths_strategies_academias_academia_name_index on nirvai.paths_academias_strategy (academia_name) INCLUDE (academia_name);
  CREATE INDEX paths_strategies_academias_path_name_index on nirvai.paths_academias_strategy (path_name) INCLUDE (path_name);
  CREATE INDEX paths_strategies_academias_strategy_name_index on nirvai.paths_academias_strategy (path_name) INCLUDE (strategy_name);
EOSQL
