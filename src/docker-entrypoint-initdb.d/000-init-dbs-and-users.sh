...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
	...
	CREATE USER $DEFAULT_DB_USER WITH PASSWORD '$DEFAULT_DB_USER_PW';
	GRANT readonly TO $DEFAULT_DB_USER;

	CREATE USER $DEFAULT_DB_ADMIN WITH PASSWORD '$DEFAULT_DB_ADMIN_PW';
	GRANT readwrite TO $DEFAULT_DB_ADMIN;
