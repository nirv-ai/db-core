...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$DEFAULT_DB" <<-EOSQL
  ...
  CREATE INDEX paths_academia_academia_name_index on nirvai.paths_academia (academia_name) INCLUDE (academia_name);
  CREATE INDEX paths_academia_path_name_index on nirvai.paths_academia (path_name) INCLUDE (path_name);
EOSQL
