...
import {
  transform,
  resetScores,
} from "../../src/docker-entrypoint-initdb.d/fixtures/transform";
...
export const hydrate = async ({
    ...
    case "activity": {
      ...
      if (status?.init !== "complete") {
        ...
        update({
          ...
          .catch((e) =>
            console.error(`error initializing ${dbName}:\n ${e.message}`)
          );
      }
    }
  }
  ...
  if (dbName in did && !did[dbName]) {
    did[dbName] = true;
    const upsertFn = dbName === "activity" ? upsertActivity : upsertSkillz;
